...
const ProductCard = ({ img, title }) => (
  ...
  <div className="relative h-64">
    <img src={img} alt={`Product: ${title}`} className="w-full h-full object-cover" loading="lazy" />
  </div>

  <div className="p-6 bg-white-100">
    <h3 className="text-xl font-semibold mb-2 hover:text-[#EC4899] transition-colors duration-300">
      {title}
    </h3>
      ...
      >
        <div className="bg-pink-100/90 p-8 md:p-12 rounded-lg backdrop-blur-sm">
          ...
          <Link to="/about">
            <motion.button
              whileHover={{ scale: 1.1 }}
              className="bg-black text-white hover: px-4 py-2 rounded uppercase text-sm tracking-wider"
            >
              Learn More
            </motion.button>
          </Link>
